...
const Card: React.FC<CardProps> = ({ activity, timeframe, previousLabel }) => {
  ...
  return (
    <div className="card-wrapper">
      <div className={`card ${getActivityClass()}`}>
        <div className="card-header">
          <h3 className="card-title">{activity.title}</h3>
          <img src={getIconPath()} alt={`${activity.title} icon`} className="card-icon" />
        </div>
        <div className="card-content">
          <p className="current-time">{data.current}hr{data.current !== 1 ? 's' : ''}</p>
          <p className="previous-time">{previousLabel} - {data.previous}hr{data.previous !== 1 ? 's' : ''}</p>
        </div>
      </div>
    </div>
  )
